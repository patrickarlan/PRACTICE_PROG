...
export const ColumnsButton = (props: ColumnsButtonProps) => {
  const { className, storeKey: _storeKey, ...rest } = props;
  const resource = useResourceContext(props);
  const storeKey = _storeKey || `${resource}.datatable`;

  const [hiddenColumns] = useStore<string[]>(storeKey, []);
  const hiddenCount = Array.isArray(hiddenColumns) ? hiddenColumns.length : 0;
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          {isMobile ? (
            <Tooltip>
              <TooltipTrigger asChild>
                ...
                >
                  ...
                  {hiddenCount > 0 && (
                    <Badge
                      className="absolute -top-1 -right-1 h-4 min-w-4 px-1 flex items-center justify-center text-[10px] rounded-full bg-primary text-primary-foreground"
                    >
                      {hiddenCount}
                    </Badge>
                  )}
            ...
            <Button variant="outline" className="cursor-pointer gap-2" {...rest}>
              <Columns className="size-4" />
              {title}
              {hiddenCount > 0 && (
                <Badge
                  className="h-5 min-w-5 px-1.5 flex items-center justify-center text-[10px] rounded-full font-bold bg-primary text-primary-foreground"
                >
                  {hiddenCount}
                </Badge>
              )}
...
export const ColumnsSelector = ({ children }: ColumnsSelectorProps) => {
  ...
  const [, setHiddenColumns] = useStore<string[]>(
    storeKey,
...
>({
  ...
  const isColumnHidden = Array.isArray(hiddenColumns) && hiddenColumns.includes(colKey);
  ...
  return isColumnFiltered ? (
    <FieldToggle
      ...
      onToggle={() => {
        ...
        if (isColumnHidden) {
          setHiddenColumns(currentHidden.filter((column) => column !== colKey));
        } else {
          setHiddenColumns([...currentHidden, colKey]);
        }
      }}
      onMove={handleMove}
    />
