...
function parserForArrayFormat(options) {
	...
	switch (options.arrayFormat) {
		...
		case "separator": return (key, value, accumulator) => {
			accumulator[key] = typeof value === "string" && value.includes(options.arrayFormatSeparator) ? value.split(options.arrayFormatSeparator).map((item) => decode(item, options)) : value === null ? value : decode(value, options);
		};
...
function exclude(input, filter, options) {
	return pick(input, Array.isArray(filter) ? (key) => !filter.includes(key) : (key, value) => !filter(key, value), options);
}
//#endregion
//#region node_modules/query-string/index.js
var query_string_default = base_exports;
//#endregion
export { query_string_default as default };
